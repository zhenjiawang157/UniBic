...
void read_discrete (FILE* fp)
{
	...
	while (getline(&line, &n, fp)>=0)
	{
		...
		while (atom != NULL)
		{	
			charset_add(symbols, atoi(atom));
			arr_c[row-1][col] =atoi(atom);
			atom = strtok(NULL, delims);
			if (++col == cols) break;
		}
		if (++row == rows+1) break;
...
discrete dis_valueA(float current, int divided,float *data)
{
	...
	for(i=0; i< divided; i++)
	{
		if(current >= quantile_from_sorted_data(data, cols, 1.0 - d_space * (i+1)))
			return i+1;
	}
	return 0;
}
void discretize (const char* stream_nm)
{
	...
	else
	{
		...
		for (row = 0; row < rows; row++)
		{
			...
			for (col = 0; col < cols; col++)
			{
	    			arr[row][col] = dis_valueA(arr[row][col],po->DIVIDED, rowdata);
			}
		}
		...
		for (row = 0; row < rows; row++)
		{
			for (col = 0; col < cols; col++) 
				rowdata[col] = arr[row][col];
			qsort(rowdata, cols, sizeof *rowdata, compare_continuous);
			upper = quantile_from_sorted_data(rowdata,cols,1-po->QUANTILE); 
			lower = quantile_from_sorted_data(rowdata,cols,po->QUANTILE);
			...
			for (col = 0; col < cols; col++)
			{
				if(arr[row][col] < lower)
		    		{
					arr_c[row][col] = arr[row][col];
					cntl++;
				}
				if(arr[row][col] > upper)
				{
					arr_c[row][col] = arr[row][col];
					cntu++;
				}
...
/*When po->QUANTILE = 0.5,using this function*/
/*void discretizeA()
{
	int row, col;
	init_dis();
...
void discretizeS(const char* stream_nm)
{
	...
	for (row = 0; row < rows; row++)
	{
		...
		for (col = 0; col < cols; col++)
		{
	    		arr_c[row][col] = dis_value(arr[row][col],po->DIVIDED, small, cntl, big, cntu);
		}
		fprintf(fw, "row %s :low=%2.5f, up=%2.5f; %d down-regulated,%d up-regulated\n", genes[row], lower, upper,cntl, cntu);
    	}
...
void discretizeSq()
{
	...
	for (row = 0; row < rows; row++)
	{
		...
		for (col = 0; col < cols; col++)
		{
		/*	charset_add(symbols, dis_valueA(arr[row][col],po->DIVIDED, small, cntl, big, cntu));*/
	    		arr_c[row][col] = dis_valueA(arr[row][col],po->DIVIDED, rowdata);
		}
    	}
	for(i=0;i<rows;i++)
		free(arr[i]);
